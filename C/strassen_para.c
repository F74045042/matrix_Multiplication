...
int main(int argc, char *argv[]){
	...
	printf("Parallel Strassen's Algorithm (cache - friendly): %lf sec.\n", total_time);
...
void strassen_matmul(int size, double **fMat, double **sMat, double **ansMat)
{
	...
	if(size <= (row/4)){
		...
		// 		ans11[i][j] = M1[i][j] + M4[i][j] - M5[i][j] + M7[i][j];
		// 		ans12[i][j] = M3[i][j] + M5[i][j];
		// 		ans21[i][j] = M2[i][j] + M4[i][j];
		// 		ans22[i][j] = M1[i][j] - M2[i][j] + M3[i][j] + M6[i][j];
		// 	}
...
void mat_mul(int size, double **fMat, double **sMat, double **ansMat)
{
	...
	// for(int ii=0; ii<size; ii+=BLK_SIZE)
	// 	for(int kk=0; kk<size; kk+=BLK_SIZE)
	// 		for(int jj=0; jj<size; jj+=BLK_SIZE)
	// 			for(int i=ii; i<min(size, ii+BLK_SIZE); i++)
	// 				for(int k=kk; k<min(size, kk+BLK_SIZE); k++)
	// 					for(int j=jj; j<min(size, jj+BLK_SIZE); j++)
